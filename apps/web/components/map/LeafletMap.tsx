...
import { FeedLocation, Location, LocationWithoutTime } from '@safe-travels/models';
...
const Marker: FC<{
  ...
  return (
    <CircleMarker
      ...
      weight={2}
      fillColor={isNewest ? colorScheme.mid : isToday ? colorScheme.light : colorScheme.grey}
...
const MapPosition: FC<{ mapPosition: [number, number]; key?: string }> = ({ mapPosition }) => {
  const map = useMap();
...
export interface Props {
  ...
  mapFocus: { key: string | undefined; position: LocationWithoutTime } | undefined;
}
...
const Map: FC<Props> = ({ locations, feedLocations, mapFocus }) => {
  ...
  return (
    isClient && (
      <MapContainer style={{ height: '100%', width: '100%', borderRadius: 20 }} center={mapPosition} zoom={9}>
